...
#define VOLUME_POT_PIN 35          // Pin where the potentiometer is connected
...
int currentVolume = 30;    // Initial volume (max)
int previousVolume = -1;   // To track changes in volume
...
void setup()
{
    ...
    pinMode(VOLUME_POT_PIN, INPUT);                 // Initialize the potentiometer pin
...
void adjustVolume()
{
    int potValue = analogRead(VOLUME_POT_PIN);         // Read the potentiometer value
    currentVolume = map(potValue, 0, 4095, 0, 30);    // Map the value to DFPlayer volume range (0–30)

    // Update the volume only if it has changed
