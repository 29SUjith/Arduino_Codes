...
const int trigPin = 13;  // Ultrasonic sensor Trig pin
    ...
    if (distance < 5) {  // Stop motor if an obstacle is detected within 5 cm
        ...
        if (Serial.available() > 0) {  // Resume motor on serial input
          ...
          if (shortcode == "N") {
              ...
              delay(5000);
              moveServo(90);  // Smooth servo movement
              delay(200);
              digitalWrite(motor2Pin, LOW);
              digitalWrite(motor2Pin1, HIGH);
              delay(4000);
              ...
              analogWrite(speedpin, speed);
              delay(5000);
              moveServo(0);  // Smooth servo movement
              delay(200);
              digitalWrite(motor2Pin, LOW);
              digitalWrite(motor2Pin1, HIGH);
              delay(4000);
              ...
              analogWrite(speedpin, speed);
              delay(5000);
              moveServo(180);  // Smooth servo movement
              delay(200);
              digitalWrite(motor2Pin, LOW);
              digitalWrite(motor2Pin1, HIGH);
              delay(4000);
          }
          moveServo(90);  // Return to center smoothly
          ...
          motorEnabled = true;  // Allow motor to run
      }
    } 
